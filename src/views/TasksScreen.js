...
const TaskPage = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.99.3:8000/tasks/", {
  ...
  const handleUpdateTask = async (taskId) => {
    ...
    try {
      const token = await AsyncStorage.getItem("access_token");
      await axios.patch(
        `http://192.168.99.3:8000/tasks/${taskId}/`,
        { title: updatedTitle },
        { headers: { Authorization: `Token ${token}` } }
      );
      fetchTasks();
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      const token = await AsyncStorage.getItem("access_token");
      await axios.delete(`http://192.168.99.3:8000/tasks/${taskId}/`, {
        headers: { Authorization: `Token ${token}` },
      });
      fetchTasks();
    } catch (error) {
  ...
  const toggleTaskCompletion = async (task) => {
    try {
      const token = await AsyncStorage.getItem("access_token");
      await axios.patch(
        `http://192.168.99.3:8000/tasks/${task.id}/`,
